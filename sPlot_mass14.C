...
void MakePlots(RooWorkspace &myws) {
  ...
  double ymin = 0.7802;
  TLegend *leg = new TLegend(0.73, 0.77, 0.89, 0.88);
...
void MakeSignalPlot(RooWorkspace &myws) {
  ...
  frame->GetYaxis()->SetRangeUser(Ydown, Yup*10);
  delete h;
...
void MakeBkgPlot(RooWorkspace &myws) {
  ...
  TLegend *leg = new TLegend(0.72, 0.69, 0.89, 0.89);
  //leg->SetTextSize(0.03);
  leg->SetTextSize(0.035);
  leg->SetLineColor(0);
...
void MakeFit2D(RooWorkspace &myws) {
  ...
  dy1 += 0.05;
  t1->DrawLatex(0.15, 0.8 - dy1, Form("f_{B} = %.6f#pm%.6f", myws.var("fB")->getValV(), myws.var("fB")->getError()));
  dy1 += 0.05;
  /*dy1 += 0.045;
  ...
  TLegend *leg_tau = new TLegend(0.72, 0.75, 0.89, 0.88);
